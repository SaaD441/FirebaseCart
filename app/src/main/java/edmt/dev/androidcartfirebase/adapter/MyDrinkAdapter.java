package edmt.dev.androidcartfirebase.adapter;
...
import edmt.dev.androidcartfirebase.R;
import edmt.dev.androidcartfirebase.eventbus.MyUpdateCartEvent;
import edmt.dev.androidcartfirebase.listener.ICartLoadListener;
import edmt.dev.androidcartfirebase.listener.IRecyclerViewClickListener;
...
public class MyDrinkAdapter extends RecyclerView.Adapter<MyDrinkAdapter.MyDrinkViewHolder> {
    ...
    private void addToCart(DrinkModel drinkModel) {
        DatabaseReference userCart = FirebaseDatabase
                .getInstance()
                .getReference("Cart")
                .child("UNIQUE_USER_ID");   //In other projects, you will add user id here

        userCart.child(drinkModel.getKey())
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if(snapshot.exists())   //if user already has an item in the cart
                        {
                            //Just update the quantity and the totalPrice
                        ...
                        else    //If there's no item in the cart, add new
                        {
